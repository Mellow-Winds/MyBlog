...
`quote.json` 管理首页大标题；`pages_quote.json` 管理阅读栏目未选择文章时的引用。两者都是非空字符串数组，例如：
...
JSON 字符串内使用 `\n` 表示换行，双引号写成 `\"`，反斜杠写成 `\\`。页面自动解析并按纯文本显示，不支持 HTML。不要在 JSON 字符串内直接输入未经转义的实际换行。

首页每次进入重新抽取；阅读引用在生成未选文章的欢迎状态时抽取。每个文案池会先生成一份随机队列，队列全部消费完才重新洗牌，因此同一轮内不会重复；允许会话存储时，刷新会继续消费当前队列。只有一条时正常显示。引用文件加载失败会在控制台记录原因，阅读页仍显示打开文章的提示。
