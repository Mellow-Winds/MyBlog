...
await mkdir(join(root, 'friend_url'));
await mkdir(join(root, 'friend_url/icons'));
await writeFile(join(root, 'friend_url/icons/manual.png'), 'manual');
const path = join(root, 'friend_url/fr_url.json');
const entries = [
...
assert.equal((await readFile(join(root, 'friend_url', saved[1].icon))).length, 6);
